...
namespace Gameplay_N
{
	class Ball
	{
		private:
			...
			void ball_move();
			void resetBall(FloatRect _ball_bounds, FloatRect _p1PaddleBounds, FloatRect _p2PaddleBounds);
		...
		public:
			Ball();
			void update(Paddle* _player1, Paddle* _player2); // for ball movement 
			void render(RenderWindow *_game_window); // for rendering ball on the window
			void handlePaddleCollision(Paddle* _player1, Paddle* _player2);
